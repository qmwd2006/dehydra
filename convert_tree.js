...
Unit.prototype.addEnum = function (fields, type_name) {
  ...
  for each (var f in fields) {
    ls.push ("case " + f.name + ":");
    ls.push ("  return convert_enum (this, \"" + f.name + "\", " + f.value + ");")
  }
  ls.push ("default:")
...
function enum2file (enum, fname) {
  var constants = ["// generated by convert_tree.js"]
  for each (var x in enum.members) {
    constants.push ("const " + x.name + " = new EnumValue(\"" + x.name + "\", " + x.value + ")")
  }
...
function process_var(v) {
  ...
  if (v.name == "tree_code_var") {
    ...
    enum2file (v.type, "tree_code.js")
  } else if (v.type.name == "tree_code") {
    print ("got sucker " + v.name)
  } else if (v.name == "tree_code_type") {
    enum2file (v.type.type, "tree_code_class.js")
  }
